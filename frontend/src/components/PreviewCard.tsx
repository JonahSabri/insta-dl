...
// ── Single carousel slide row ─────────────────────────────────────────────────

function SlideRow({ file, index }: { file: CarouselFile; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-[10px] text-slate-600">{isVideo ? "ویدیو" : "تصویر"}</p>
      </div>
...
export default function PreviewCard({ result, onReset }: Props) {
  ...
  const meta = META[result.media_type] ?? META.unknown;
  ...
  const downloadLabel = isCarousel
    ? `دانلود ZIP (${result.file_count} فایل)`
    : isImage
      ? "دانلود تصویر"
      : "دانلود ویدیو";
  ...
  return (
    <div className="anim-bounce-in glow-card overflow-hidden">
      ...
      <div className="flex items-start gap-4 p-5">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-2.5 pt-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-green-400">آماده دانلود</span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {isCarousel && (
              <span className="badge rounded-lg border border-white/10 bg-white/5 px-2.5 py-1
                text-xs text-slate-400">
                📦 {result.file_count} فایل
              </span>
            )}
          </div>
        </div>
      ...
      {isCarousel && (
        <>
          {/* Main ZIP download + toggle individual */}
          <div className="mx-4 mb-3 space-y-2">
            ...
            {showSlides && slides.length > 0 && (
              <div className="space-y-1.5 rounded-2xl border border-white/[0.07] bg-black/20 p-3">
                <p className="mb-2 px-1 text-xs text-slate-500">دانلود هر اسلاید جداگانه:</p>
                {slides.map((file, i) => (
                  <SlideRow key={file.index} file={file} index={i} />
                ))}
              </div>
            )}
          </div>
